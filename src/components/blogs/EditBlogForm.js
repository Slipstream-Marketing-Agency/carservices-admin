...
const EditBlogForm = ({data, match}) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_API_KEY}admin/model?isAll=1`,
                {
                    headers: {
                        Authorization: "Bearer " + user.token
                    }
            ...
            .then((data) => {
                console.log('dddd ', data);
                let modelsData = data.models.map(model => (
                    {
                        value: model.id,
                        label: model.name
                    }
    ...
    useEffect(() => {
        ...
        //     bufferData.brand = brandList.filter(brand => {
        //         return brand.value == data.brand.id
        //     })[0]
        //     setBlogData(bufferData);
        // }
    ...
    const onSubmit = (values, { setSubmitting }) => {
        ...
        values.models = values.models.map(model => model.value)
